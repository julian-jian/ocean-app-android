...
import com.chenxi.tabview.adapter.TabBaseAdapter;
...
import com.sky.lamp.view.NoScrollViewPager;
...
public class TabContainerView extends RelativeLayout {
    ...
    private void initViewPager(Context context) {
        contentViewPager = new NoScrollViewPager(context);
    ...
    public void setAdapter(TabBaseAdapter tabBaseAdapter) {
        setAdapter(tabBaseAdapter, 0);
    }

    public void setAdapter(TabBaseAdapter tabBaseAdapter, int index) {
        if (tabBaseAdapter == null) return;
        tabHost.addTabs(tabBaseAdapter, textSize, textColor, selectedTextColor,drawablePadding,iconWidth,iconHeight);

        contentViewPager.setAdapter(new TabViewPagerAdapter(tabBaseAdapter.getFragmentManager(), tabBaseAdapter.getFragmentArray()));
